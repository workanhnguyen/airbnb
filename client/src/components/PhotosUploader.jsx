...
const PhotosUploader = ({ addedPhotos, setAddedPhotos }) => {
  ...
  const removePhoto = (e, filename) => {
    e.preventDefault();
    setAddedPhotos([...addedPhotos.filter((link) => link !== filename)]);
  };

  const selectAsMainPhoto = (e, filename) => {
    e.preventDefault();
    setAddedPhotos([filename, ...addedPhotos.filter((link) => link !== filename)]);
  };
      ...
      <div className="mt-2 gap-2 grid grid-cols-3 lg:grid-cols-6 md:grid-cols-4">
        {addedPhotos.length > 0 &&
          addedPhotos.map((link, index) => (
            <div
              key={index}
              className="relative flex rounded-2xl h-32 overflow-hidden border"
            >
              <img
                className="w-full object-cover"
                src={IMAGE_LOAD_BASE_URL + "/" + link}
                alt={link}
              />
              <button
